...
double compute_matrix_element_c(int iv, int in1p, double l1p, int in2p, double l2p, int jp, int lambdap, int mup, int sp, int msp, int tp, int mtp, int in1, double l1, int in2, double l2, int j, int lambda, int mu, int s, int ms, int t, int mt) {
  // Computes the two-body nuclear matrix element of the operator unity with arbitrary radial function specified by iv

  double mat = 0.0;
  if ((lambda != lambdap) || (mu != mup) || (s != sp) || (ms != msp) || (t != tp) || (mt != mtp)) {return mat;}
  mat = 0.5*(t*(t + 1.0) - 1.5);
  // Now perform Brody-Moshinsky transformation
...
double compute_matrix_element_av18(int in1p, double j1p, int in2p, double j2p, int j12p, int t12p, int in1, double j1, int in2, double j2, int j12, int t12) {
  double v = 0.0;
  v += compute_matrix_element_c(5, in1p, j1p, in2p, j2p, j12p, t12p, in1, j1, in2, j2, j12, t12);
